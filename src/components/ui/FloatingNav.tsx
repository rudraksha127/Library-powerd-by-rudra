...
export function FloatingNav() {
  ...
  return (
    ...
    >
      <Link
        href="/"
        className="focus-ring rounded-md px-1 font-display text-xs tracking-[0.35em] uppercase text-white/80 font-light hover:text-[var(--color-champagne-gold)] transition-colors"
      >
        Aurora
      </Link>
      ...
      <div className="hidden sm:flex items-center gap-1.5">
        {navLinks.map((link) => (
          <Link
            ...
            className={`focus-ring inline-block px-4 py-2 rounded-full font-body text-[9px] tracking-[0.25em] uppercase transition-all duration-500 ${
              pathname === link.href
      ...
      <button 
        onClick={toggleMusic}
        className="focus-ring h-8 pl-3 pr-4 rounded-full bg-white/[0.03] border border-white/[0.05] flex items-center gap-2 hover:bg-white/[0.08] hover:border-[var(--color-champagne-gold)]/20 transition-all duration-500"
